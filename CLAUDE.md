...
- `CHANGELOG.md` is generated by `git-cliff` (`npm run changelog`) from Conventional Commits — never hand-edit it.
- Release: bump `version` in `package.json`, `src-tauri/Cargo.toml`, `src-tauri/tauri.conf.json`, run `npm run changelog`, commit, then tag `vX.Y.Z` (annotated). Tag/push only when explicitly asked.

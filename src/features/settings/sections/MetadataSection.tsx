 ...
 *
 * `checks` is a slot rather than a component this file imports: the card holds
 * a preference owned by `features/library/triage`, and features do not import
 * each other. The shell fills it (see `app/layout/SettingsHost`), and the pane
 * renders without it anywhere else.
 */
export function MetadataSection({ checks }: { checks?: ReactNode }) {
  ...
  return (
    <>
      ...
      {/* Eight switches of its own, so it is a card and not a row. */}
      {checks}
    </>
  );
}

...
interface ImportProgressProps {
  ...
  /** Album folders the scan counted — the denominator. */
  total: number;
}
...
export function ImportProgress({ progress, total }: ImportProgressProps) {
  ...
  return (
    <div className="flex flex-col gap-2">
      <ProgressBar.Root value={percent} aria-label={t("importing")}>
        <ProgressBar.Track>
          <ProgressBar.Fill />
        </ProgressBar.Track>
      </ProgressBar.Root>
      ...
      <div className="flex flex-col gap-0.5">
        <p className="text-[0.8125rem] font-medium">{t("progress", { done, total })}</p>

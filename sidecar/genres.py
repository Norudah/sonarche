"""Recompute genres for the whole library against the current tree.

No fingerprinting or MusicBrainz: MB genres canonicalize offline, and only
genre-less items trigger a Last.fm fetch."""
...
# lastgenre's Last.fm client has no rate limiting and uses beets' shared API
# key, so network calls are paced. The host passes the user's setting.
...
def recompute(request_id: str, params: dict) -> dict:
    ...
    for done, item in enumerate(items, start=1):
        if provenance.was_hand_edited(item, "genres"):
            ...
            # An empty result keeps the existing genre.
